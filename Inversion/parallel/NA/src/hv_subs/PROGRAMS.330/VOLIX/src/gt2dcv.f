        ...
        subroutine gt2dcv(lun,mmax,listmd,xmin,xmax,zmin,zmax)
        logical listmd
        character string*80
...
c-----
c       get model specifically for 2-D isotropic constant velocity
c-----
c-----
...
c-----
        ...
        write(LOT,*)mmax, (nc(i),i=1,mmax)
        deplow = 0.0
        dephih = 0.0
        zmin = 0.0
        do 900 i=1,mmax
            read(lun,*)(xc(i,j),zc(i,j),j=1,nc(i))
            write(LOT,*)(xc(i,j),zc(i,j),j=1,nc(i))
  900   continue
        do 901 i=1,mmax
            do 902 j=1,nc(i)
                ii(i,j) = -1
