		...
		if(*cp[i]  == '-'){
			switch(cp[i][1]){
				case 'N':
					...
					if(numperpage < 1)
						numperpage = 1;
					if(numperpage > MAXNUMPERPAGE)
						numperpage = MAXNUMPERPAGE;
					break;
				case 'G':
					interstationgreen = 1;
					break;
				case 'h':
				case '?':
					fprintf(stderr,"Usage: do_mft [-Nnumberperpage] [-G] sacfiles\n");
					fprintf(stderr,"  e.g.:  do_mft -N2 -G *Z\n");
...
void DInquire_file(int aargc, char **cp)
{
	...
	gnomesort(aargc, sortfloat, sortptr) ;
}
