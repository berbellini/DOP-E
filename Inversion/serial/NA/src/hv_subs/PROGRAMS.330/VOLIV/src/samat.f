...
c-----
        subroutine samat(id,nfilt,inarg,wref,invcsl,nf1,invdep,
...
C     1     'Percent of Signal Power Fit (RFTN)     :',
C     1     (1.0 - sumrr/sumrd)*100.0, '% for',nurftn, ' RFTNs'
        if(id.ne.3)then
...
c-----
 ...
 1999   continue
Cc-----
Cc      read the receiver function information
Cc-----
C       open(2,file='tmpmrgr.9',form='unformatted',access='sequential')
C       rewind 2
Cc-----
Cc          do not have to write this since we have just done it
Cc-----
C           read(2)m2,nfilt
C           read(2)(ddd(i),i=1,m2)
C           read(2)(ddd(i),i=1,m2)
Cc-----
Cc      now write out the partials and residuals for the inversion
Cc-----
C 2000  continue
C           read(2,end=2999) (ddd(j),j=1,m2),dy
C           do 2100 j=1,m2
C               ddd(j) = ddd(j) *facr
C 2100      continue
C           dy = dy * facr
