...
c-----
c       CHANGES:
c       20 SEP 2010 - added -W flag to indicate that the waveform
c             goodness of fit stored in header variable USER file of
c             the predicted waveform be used as a weight
c             In this the input is
c                Azimuth Delay Weight
c             instead of
c                Azimuth Delay
c             The weight is assumed to vary between 0 and 100. A -12345
c             indicates that no weighting is done
c-----
...
c       o The velocity model used for the inversion is incorrect
...
c          time_shift = A + C sin Az  with C positive
...
c-----
        ...
        real azobs(1000), delayobs(1000), vrtovl(1000), wt(1000)
        logical dowt
c-----
...
c-----
        ...
        sumd = 0.0
        sumwt = 0.0
        call fill('Z.dat',1.0,wt,a,y,n,sumd,sumwt
     1      ,azobs,delayobs,vrtovl,dowt)
        call fill('R.dat',1.0,wt,a,y,n,sumd,sumwt
     1      ,azobs,delayobs,vrtovl,dowt)
        call fill('T.dat',0.92,wt,a,y,n,sumd,sumwt
     1      ,azobs,delayobs,vrtovl,dowt)
c-----
...
c-----
        ...
        write(6,*)'Rayl shift (km)  :',sqrt(x(2)*x(2) + x(3)*x(3))*3.2
        write(6,*)'Average delay    :',sumd/sumwt
        write(6,*)'T0SHIFT=',x(1)
...
c-----
     ...
     1        delayobs(i)-tpre
        ...
        subroutine fill(fname,vrvl,wt,a,y,n,sumd,sumwt
     1      ,azobs,delayobs,vrtovl,
     1       dowt)
        character fname*(*)
        real vrvl
        real a(3,3), y(3)
        real azobs(*), delayobs(*), vrtovl(*), wt(*)
        real sumd, sumwt
        logical dowt
        integer n
        real az, delay,weight
        logical nowt
        logical ext
        nowt = .false.
        nlow = n + 1
 ...
 1001       continue
            if(dowt)then
                 read(1,*,end=1002)az,delay,weight
                 weight = 1.0
            else
 ...
 1002   continue
            close(1)
c-----
c        now fill up the least square
c        taking care if a -12345 was seen, e.g., weight < -100
c-----
            do 2002 i=nlow,n
               az = azobs(i)
               delay = delayobs(i)
...
c-----
               saz = saz * vrvl
               caz = caz * vrvl
               a(1,1) = a(1,1) + 1.0*1.0*w
               a(1,2) = a(1,2) + caz*1.0*w
               a(1,3) = a(1,3) + saz*1.0*w
               a(2,1) = a(2,1) + 1.0*caz*w
               a(2,2) = a(2,2) + caz*caz*w
               a(2,3) = a(2,3) + saz*caz*w
               a(3,1) = a(3,1) + 1.0*saz*w
               a(3,2) = a(3,2) + caz*saz*w
               a(3,3) = a(3,3) + saz*saz*w
               y(1) = y(1) + delay * 1.0*w
               y(2) = y(2) + delay * caz*w
               y(3) = y(3) + delay * saz*w
               sumd = sumd + delay*w
...
c
c-----
c       dowt   L    - .true. use weighted least squares
c-----
        logical dowt
        character*25 name
        integer*4 mnmarg
   ...
   13   continue
        ...
        subroutine usage()
        parameter (LER=0, LIN=5, LOT=6)
        WRITE(LER,*)'Usage: dowt -W'
        write(LER,*)
     1  ' -W       (default false)  use weighting'
        write(LER,*)
     1  ' -?                   This online help'
        write(LER,*)
