...
c et al., GJI).
c 20190305: dop-e_v1.1.f increase stability for circular polarization.
c 20190717: dop-e_v1.2.f permit log sampling of frequency.
c 
c Further:
c ========
...
c	Scaling the vertical component by a factor BB (ABVersion)
	trace1 = BB*trace1
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
c================= ABVersion ====================================
c       Scaling zdeg considering that the vertical axis is multiplied by BB
	zdeg=atan(tan(zdeg/rad2deg)/BB)*rad2deg
	wdeg=atan(tan(wdeg/rad2deg)/BB)*rad2deg
c================================================================
...
c  angle of semi major with Z and H:
            if (wdeg.lt.90.and.linear(ns,nf).gt.0.05) then
              aaaa=rsig1(ns)
              aaa2=asin(aaaa)
              aaa2=abs(aaa2)
...
c	--------------AB version------------------
     ...
     & linear(ns,nf),r1(ns,nf,1),r1(ns,nf,2),r1(ns,nf,3),
     & r3(ns,nf,1),r3(ns,nf,2),r3(ns,nf,3),BB*rhv,rba
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     & zdeg,wdeg,lhv,lsemi,lzwout,lflog)
c  Author: M. Schimmel
c===============================================================
c
      character*80 name1, name2, name3
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      write(*,*)
      write(*,*)'USAGE:  dop-e_v1.1 in1 in2 in3  [parameters]'
      write(*,*)
...
c     write(*,*)'  ntr   :  # of traces in bin files.'
     ...
     & 'frequency'
      write(*,*)'           (time, DOP). Does not yet work with log.'
     ...
     & 'zdeg deg.'
      ...
      write(*,*)'           zdeg=90 or more will deactivate this '// 
     & 'option.'
      write(*,*)'  wdeg= :  Set DOP to zero whenever the semi-major'//
     & ' vector '
      write(*,*)'           deviates from the horizontal plane or '//
     & 'vertical ax'
      write(*,*)'           by more than wdeg deg. Note that the wdeg'//
     & ' option '
      write(*,*)'           makes no sense for circular motion. It is'//
     & ' therefore'
      write(*,*)'           disabled whenever linearity < 0.05.'//
     & ' Default: wdeg=15.'
      write(*,*)'           wdeg=90 or more will deactivate this '// 
     & 'option.'
     ...
     & ' improvements, collaborations,'
      ...
      write(*,*)' '
      write(*,*)'LAST MODIFICATION: 17.07.2019 '
      write(*,*)' '
